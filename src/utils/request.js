...
instance.interceptors.response.use(function (response) {
  // 对响应数据做点什么
  if (response.data && response.data.code !== '0' && response.data.info) {
    Notification({
      type: 'error',
  ...
  if (error.toString().includes(loginFailErrorStr)) {
    ...
    const jumpToUrl = location.origin + location.pathname + '#/?redirecturl=' + encodeURIComponent(location.hash.substring(1))
