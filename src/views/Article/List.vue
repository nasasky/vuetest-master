<template>
  <div class="article-list-container">
    ...
    <div v-show="getSearchForm.state !== '-3'">
      <el-input @input="changeKeywords" size="small" style="width: 300px;margin-right:20px;" placeholder="通过标题搜索该分类下的文章" prefix-icon="el-icon-search" :value="getSearchForm.keywords" ></el-input>
      <el-button size="small" type="primary" @click="search">搜索</el-button>
      <!-- 全部文章走这个，不要那个勾选框 -->
      <el-table v-show="getSearchForm.state === '-1'" :data="dataList" style="margin-top:20px;">
        <el-table-column label="标题" prop="title">
          <template slot-scope="scope">
            <router-link :to="{path: '/article/detail/' + scope.row.id}">{{scope.row.title}}</router-link>
          </template>
        </el-table-column>
        <el-table-column v-for="column in columns" :key="column.prop" :prop="column.prop" :label="column.label"></el-table-column>
      </el-table>
      <el-table v-show="getSearchForm.state !== '-1'" :data="dataList" @selection-change="handleSelectionChange" style="margin-top:20px;">
        ...
        <el-table-column label="操作" v-show="isCanDelete">
          <template slot-scope="scope">
            <el-button v-show="isCanEdit" size="small" @click="jumpToModifyPage(scope.row.id)">编辑</el-button>
      ...
      <el-button v-if="isCanDelete" size="small" type="danger" @click="betchDelete">批量删除</el-button>
    </div>
...
export default {
  ...
  methods: {
    ...
    search() {
      ...
      if (this.getSearchForm.state === "-2") { // 博客管理只有admin才能有权限看到
