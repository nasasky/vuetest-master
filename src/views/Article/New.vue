<template>
  <div class="new-article-container">
    <el-select v-model="form.cid" placeholder="请选择栏目">
      <el-option :key="item.id" v-for="item in catogoryList" :value="item.id" :label="item.cateName">
      </el-option>
    </el-select>
    <el-input placeholder="请输入标题" v-model="form.title" style="width:400px;margin:0 20px;"></el-input>
    <el-tag class="tag" v-for="(item, index) in form.dynamicTags" :key="index" closable @close="handleCloseTag(index)">{{item.tagName}}</el-tag>
    <el-input @keyup.enter.native="pushTagToArr" @blur="pushTagToArr" v-show="isInputTag" v-model="tagName" style="width:80px;"></el-input>
    <el-button v-show="!isInputTag" @click="addTag">+Tag</el-button>
    <div class="mavon-editor-container">
      <mavon-editor ref="md" @imgAdd="imgAdd" style="height:100%" v-model="form.mdContent"></mavon-editor>
    </div>
...
export default {
  ...
  methods: {
    ...
    imgAdd(pos, $file) {
      ...
      request.post('/article/uploadimg', formData, {
        headers: {
