<template>
  <div class="user-list-container">
    <div style="text-align:center;margin-bottom:15px;">
      <p style="color:#ccc;font-size:13px;text-align:center;margin-bottom:10px;">温馨提示：这里最多展示20个用户，没看到的用户，请输入昵称查询</p>
      <el-input size="small" placeholder="请输入昵称查询" style="width:400px" v-model="form.nickname"></el-input>
    ...
    <section class="card-container">
      <el-card v-for="(item, index) in tableList" :key="item.id">
        <div slot="header" class="card-header">
          <b>{{item.nickname}}</b>
          <span style="color:#f56c6c;" @click="handleDelete(item.id)"><i class="el-icon-delete"></i>删除</span>
        </div>
        ...
        <p>
          用户状态：
          {{item.enabled ? '启用' : '禁用'}}
          <el-switch :value="item.enabled" @change="val => changeEnable(val,index)" active-color="#13ce66" inactive-color="#ccc"></el-switch>
        </p>
        <p>
          用户角色：
  ...
  export default {
    ...
    methods: {
      ...
      changeEnable(enabled, index) {
        ...
        request.put('/admin/user/enabled', {
          enabled,
          uid
        },  {}, true).then(res => {
          if (res.code === '0') {
